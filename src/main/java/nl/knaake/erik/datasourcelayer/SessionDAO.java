...
public class SessionDAO extends BasicDAO implements ISessionDAO {
    ...
    private CacheIdentityMap<String, SessionExpirationDTO> sessionCache;

    /**
     * Initializes the session cache
     */
    public SessionDAO() {
        sessionCache = CacheIdentityMap.getInstance(String.class, SessionExpirationDTO.class);
    }
    ...
    public void extendSession(String token, int sessionTimeoutTime) {
        ...
        try {
            ...
            SessionExpirationDTO sessionToExtend = (SessionExpirationDTO) sessionCache.get(token);
            if(sessionToExtend == null) {
                sessionToExtend = new SessionExpirationDTO(token, DateConverter.convertDateToDateTimeString(new Date()));
            ...
            sessionCache.update(token, sessionToExtend);
            stmt.executeUpdate();
            stmt.close();
        }
    ...
    private SessionCacheState isSessionValidFromCache(String token) {
        SessionExpirationDTO cachedSession = (SessionExpirationDTO) sessionCache.get(token);
