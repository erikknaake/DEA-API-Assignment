...
public class TrackInPlaylistDAO extends TrackParsingDAO implements ITrackInPlaylistDAO {
    ...
    private CacheIdentityMap<Integer, TrackCollectionDTO> trackInPlaylistCache;

    /**
     * Initializes the trackInPlaylistCache
     */
    public TrackInPlaylistDAO() {
        trackInPlaylistCache = CacheIdentityMap.getInstance(Integer.class, TrackCollectionDTO.class);
    }
    ...
    public TrackCollectionDTO getAllTracksFromPlaylist(int playlistId) {
        TrackCollectionDTO tracks = (TrackCollectionDTO) trackInPlaylistCache.get(playlistId);
        if(tracks != null) {
            return tracks;
        }
        ...
        try {
            ...
            trackInPlaylistCache.put(playlistId, tracks);
            set.close();
        }
    ...
    private void removeTrackFromCache(int playlistId, int trackId) {
        TrackCollectionDTO trackCollectionDTO = (TrackCollectionDTO) trackInPlaylistCache.get(playlistId);
        if(trackCollectionDTO != null) {
            trackCollectionDTO.removeTrack(trackId);
            trackInPlaylistCache.update(playlistId, trackCollectionDTO);
        }
    }
