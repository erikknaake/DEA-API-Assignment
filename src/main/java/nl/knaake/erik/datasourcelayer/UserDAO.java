...
public class UserDAO extends BasicDAO implements IUserDAO {
    ...
    private void addNewSessionToCache(String token) {
        sessionCache.put(token, new SessionExpirationDTO(token, DateConverter.convertDateToDateTimeString(DateConverter.addMinutes(new Date(), sessionHandler.getTimeoutTime()))));
    }
    ...
    private void deleteOldSessionIfExists(String user) throws SQLException {
        ...
        if(hasSessionSet.next()) {
            ...
            deleteSession.close();
            sessionCache.delete(hasSessionSet.getString("sessionToken"));
        }
        getDb().closeConnection();
    }
}
